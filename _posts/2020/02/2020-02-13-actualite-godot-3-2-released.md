...
La communauté de développeurs autour de Quest VR est en plein essor. Il existe déà de nombreux prototypes intéressants qui utilisent Godot, et nous pensons que la version 3.2 devrait booster encore plus cette communauté. Restez à l'affût des mises à jour car Bastiaan va bientôt envoyer des plugins VR précompilés pour tous les casques VR pris en charge dans l'[Asset lib](https://godotengine.org/asset-library/asset). Puis il publiera probablement une mise à jour avec des instructions sur la façon d'utiliser le plugin.
